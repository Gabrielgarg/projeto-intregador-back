...
import { TokenPayloadPost } from '../models/Posts'
...
export class TokenManager {
    ...
    public createTokenPost = (payload: TokenPayloadPost): string => {
        const token = jwt.sign(
            payload,
            process.env.JWT_KEY as string,
            {
    ...
    public getPayloadPost = (token: string): TokenPayloadPost | null => {
        try {
            const payload = jwt.verify(
                token,
                process.env.JWT_KEY as string
            ...
            return payload as TokenPayloadPost
